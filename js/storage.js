...
const DB = (() => {
    ...
    const setSession = (userId) => write(K.SESSION, { userId, createdAt: new Date().toISOString() });
    ...
    return {
        K, users, findUserByEmail, upsertUser,
        getSession, setSession, clearSession, requireAuth,
        getAddress, setAddress,
        getOrders, addOrder,
        getLegacyCart, setLegacyCart, clearLegacyCart,
        getOrderById(userId, orderId) { return (read('serenne:orders', {})[userId] ?? []).find(o => o.id === orderId) || null; },
        updateUser(u) { return this.upsertUser(u); },
