...
document.addEventListener('DOMContentLoaded', () => {
    ...
    <div class="order-item">
      ...
      <div class="order-status ${o.status === 'Entregue' ? 'status-delivered' : (o.status === 'Enviado' ? 'status-shipped' : 'status-processing')}">${o.status}</div>
      <div>${brl(o.total)}</div>
      <a class="order-link" href="/html/pedido-detalhe.html?id=${encodeURIComponent(o.id)}">Ver detalhes</a>
    </div>
  `).join('');
});
