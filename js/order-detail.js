...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!session?.userId) {
        location.href = '/html/login.html?next=/html/pedido-detalhe.html' + location.search;
        return;
    }
    ...
    if (!id) {
        document.querySelector('.order-detail').innerHTML = '<p>Pedido não encontrado.</p>';
        return;
    }
    ...
    <li class="order-product">
      ...
      <div class="info">
        ...
        <div class="meta">Qtd: ${it.qty ?? 1} • Preço: ${Number(it.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
      </div>
    </li>
  `).join('');
    ...
    <div class="order-head">
      ...
      <div class="muted">Realizado em ${new Date(order.createdAt).toLocaleString('pt-BR')}</div>
      <div class="status"><strong>Status:</strong> ${order.status}</div>
      <div class="total"><strong>Total:</strong> ${Number(order.total).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
    </div>
